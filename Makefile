...
help: ## Show this help message
	@echo ""
	@echo "$(CYAN)╔═══════════════════════════════════════════════════════════════════╗$(NC)"
	@echo "$(CYAN)║           BlackieFi 3.0 - Development Commands                    ║$(NC)"
	@echo "$(CYAN)╚═══════════════════════════════════════════════════════════════════╝$(NC)"
	@echo ""
	@echo "$(YELLOW)QUICK START:$(NC)"
	@echo "  $(GREEN)make setup$(NC)          - First-time setup (install all dependencies)"
	@echo "  $(GREEN)make dev$(NC)            - Start backend + frontend for development"
	@echo "  $(GREEN)make docker-up$(NC)      - Start all services with Docker"
	@echo ""
	@echo "$(YELLOW)DEVELOPMENT (No Docker):$(NC)"
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  $(GREEN)%-18s$(NC) %s\n", $$1, $$2}' $(MAKEFILE_LIST) | grep -E "^  (backend|frontend|db|dev|setup|stop|clean|test|lint)"
	@echo ""
	@echo "$(YELLOW)DOCKER COMMANDS:$(NC)"
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  $(GREEN)%-18s$(NC) %s\n", $$1, $$2}' $(MAKEFILE_LIST) | grep -E "^  docker"
	@echo ""
	@echo "$(YELLOW)MICROSERVICES:$(NC)"
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  $(GREEN)%-18s$(NC) %s\n", $$1, $$2}' $(MAKEFILE_LIST) | grep -E "^  microservices"
	@echo ""
	@echo "$(YELLOW)OBSERVABILITY:$(NC)"
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  $(GREEN)%-18s$(NC) %s\n", $$1, $$2}' $(MAKEFILE_LIST) | grep -E "^  observability"
	@echo ""
	@echo "$(YELLOW)ALL SERVICES:$(NC)"
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z_-]+:.*?## / {printf "  $(GREEN)%-18s$(NC) %s\n", $$1, $$2}' $(MAKEFILE_LIST) | grep -E "^  all-"
	@echo ""
