...
class MarketDataService:
    ...
    async def get_crypto_price(self, coin_id: str) -> Dict[str, Any]:
        ...
        # Check if API key is configured (required to avoid rate limits)
        api_key = await self._get_api_key(MarketDataProvider.COINGECKO)
        if not api_key:
            return {"error": "CoinGecko API key not configured. Please add your API key in Market Data Settings to avoid rate limits.", "requires_api_key": True}
