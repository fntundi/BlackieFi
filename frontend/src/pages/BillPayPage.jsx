...
import {
  ...
  RefreshCw, Pencil, History, CreditCard
} from "lucide-react";

const BillPayForm = ({ form, setForm, editId, accounts, categories, onSubmit, onCancel }) => (
  <div className="billpay-form-card" data-testid="billpay-form">
    ...
    <form onSubmit={onSubmit}>
      ...
      <div className="form-actions">
        ...
        <button type="button" className="btn-sm btn-ghost" onClick={onCancel} data-testid="cancel-schedule-btn">Cancel</button>
      </div>
    </form>
  </div>
);

const ScheduleCard = ({ s, processing, onPayNow, onToggle, onEdit, onDelete }) => {
  const isDue = s.enabled && s.next_payment_date <= new Date().toISOString().slice(0, 10);
  return (
    <div className={`billpay-card ${!s.enabled ? "disabled" : ""} ${isDue ? "due" : ""}`} data-testid={`schedule-${s.id}`}>
      ...
      <div className="billpay-card-actions">
        ...
        <div className="billpay-btns">
          <button className="btn-icon-sm" title="Pay Now" onClick={() => onPayNow(s.id)}
          ...
          <button className="btn-icon-sm" title={s.enabled ? "Disable" : "Enable"}
            onClick={() => onToggle(s.id)} data-testid={`toggle-${s.id}`}>
            {s.enabled ? <Power size={14} className="text-green" /> : <PowerOff size={14} className="text-muted" />}
          </button>
          <button className="btn-icon-sm" title="Edit" onClick={() => onEdit(s)} data-testid={`edit-${s.id}`}>
            <Pencil size={14} />
          </button>
          <button className="btn-icon-sm btn-danger-icon" title="Delete" onClick={() => onDelete(s.id)} data-testid={`delete-${s.id}`}>
            <Trash2 size={14} />
          </button>
        </div>
      </div>
...
const EMPTY_FORM = { name: "", amount: "", frequency: "monthly", day_of_month: 1, account_id: "", category_id: "", source_type: "expense", enabled: true };
...
export default function BillPayPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load bill pay data:", e.message);
    }
    setLoading(false);
  }, []);
  ...
  const startEdit = (s) => {
    setForm({ name: s.name, amount: s.amount, frequency: s.frequency, day_of_month: s.day_of_month,
      account_id: s.account_id || "", category_id: s.category_id || "", source_type: s.source_type || "expense", enabled: s.enabled });
    setEditId(s.id);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) { await api.put(`/billpay/schedules/${editId}`, payload); }
      else { await api.post("/billpay/schedules", payload); }
      resetForm(); fetchData();
    } catch (err) { alert(err.response?.data?.detail || "Error saving schedule"); }
  };
  ...
  const handlePayNow = async (id) => {
    setProcessing(id);
    try { const res = await api.post(`/billpay/schedules/${id}/pay-now`); alert(res.data.message); fetchData(); }
    catch (err) { alert(err.response?.data?.detail || "Error"); }
    setProcessing(null);
  };
  ...
  const handleProcessDue = async () => {
    ...
    setProcessing("all");
    try { const res = await api.post("/billpay/process-due"); alert(res.data.message); fetchData(); }
    catch (err) { alert(err.response?.data?.detail || "Error"); }
    setProcessing(null);
  };

  const today = new Date().toISOString().slice(0, 10);
  const dueCount = schedules.filter(s => s.enabled && s.next_payment_date <= today).length;
  ...
  return (
    <div className="page-content" data-testid="billpay-page">
      ...
      <div className="stats-grid" data-testid="billpay-stats">
        <div className="stat-card"><div className="stat-label">Active Schedules</div><div className="stat-value">{activeCount}</div></div>
        <div className="stat-card"><div className="stat-label">Due Now</div><div className="stat-value" style={{ color: dueCount > 0 ? "var(--warning)" : "var(--success)" }}>{dueCount}</div></div>
        <div className="stat-card"><div className="stat-label">Monthly Total</div><div className="stat-value">${totalMonthly.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div></div>
        <div className="stat-card"><div className="stat-label">Total Schedules</div><div className="stat-value">{schedules.length}</div></div>
      </div>

      {dueCount > 0 && (
        <div className="billpay-alert" data-testid="due-alert">
          <div className="alert-content"><CreditCard size={20} /><span><strong>{dueCount} bill(s)</strong> are due for payment</span></div>
      ...
      {showForm && <BillPayForm form={form} setForm={setForm} editId={editId} accounts={accounts} categories={categories} onSubmit={handleSubmit} onCancel={resetForm} />}
            ...
            <div className="billpay-list" data-testid="schedules-list">
              {schedules.length === 0 ? (
                <div className="empty-state" data-testid="empty-schedules">
                  <CalendarClock size={48} className="text-muted" /><h3>No Bill Schedules</h3><p>Create your first automated bill payment schedule</p>
                </div>
              ) : schedules.map(s => (
                <ScheduleCard key={s.id} s={s} processing={processing} onPayNow={handlePayNow} onToggle={handleToggle} onEdit={startEdit} onDelete={handleDelete} />
              ))}
            </div>
          )}
          {tab === "history" && <PaymentHistoryTable history={history} />}
        </>
      )}
    </div>
  );
